...
ggplot( these_fsts, aes( x=n_var, col=comp ) ) + geom_density( fill="grey" ) + ylim(0, 0.01) + facet_wrap( ~comp )

ggplot( these_fsts, aes( y=fst, x=n_var, col=comp ) ) + geom_point() + facet_wrap( ~comp )

ggplot( these_fsts, aes( x=scaf_bin, y=n_var ) ) + geom_point()
...
group_by( these_fsts, scaf ) %>% summarise( n_bins=n_distinct( scaf_bin ) ) %>% arrange( desc( n_bins )) %>% summary()
...
fltr_fsts <- filter( these_fsts, scaf %in% droplevels(by_scaf$scaf) ) %>%  # dumping shorter scafs
              filter( fst > quantile( these_fsts$fst, probs=0.9 ) ) %>%  # dumping low fst scafs
              filter( scaf != "Scaffold1910" & scaf != "Scaffold3315" & 
                        scaf != "Scaffold3294" & scaf != "Scaffold24" )  # dumping 4 scafs with contamination

ggplot( fltr_fsts, aes( x=fst, col=comp ) ) + geom_density( fill="grey" ) + facet_wrap( ~comp )
